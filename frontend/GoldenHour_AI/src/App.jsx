...
import {
  ...
  useSelectedHospital,
} from "./hooks/useAmbulanceTracking.js";
...
function Dashboard() {
  ...
  const { data: selectedHospitalFromBackend } = useSelectedHospital(emergencyId);
  ...
  useEffect(() => {
    ...
    console.log("selectedHospitalData (UI):", selectedHospitalData);
    console.log("ambulanceData:", ambulanceData);
    console.log("selectedHospitalFromBackend:", selectedHospitalFromBackend);
    console.log("==================");
  }, [
    emergencyId,
    triageData,
  ...
  // Called by EmergencyForm when /triage succeeds
  ...
  const handleHospitalSelection = (hospital) => {
    ...
    console.log("✅ Hospital selected (from list):", hospital);
  };
  ...
  // Decide which hospital data to use on the map:
  // prefer backend-selected hospital if available, otherwise the one user clicked
  const effectiveHospital =
    selectedHospitalFromBackend || selectedHospitalData || null;

  const hasValidEmergencyLocation =
    triageData &&
    triageData.location &&
    typeof triageData.location.lat === "number" &&
    typeof triageData.location.lng === "number";

  const hasValidHospitalLocation =
    effectiveHospital &&
    (typeof effectiveHospital.latitude === "number" ||
      typeof effectiveHospital.lat === "number") &&
    (typeof effectiveHospital.longitude === "number" ||
      typeof effectiveHospital.lng === "number");

  const hasValidAmbulanceLocation =
    ambulanceData &&
    typeof ambulanceData.currentLat === "number" &&
    typeof ambulanceData.currentLng === "number";

  const shouldShowMap =
    emergencyId &&
    hasValidEmergencyLocation &&
    needsAmbulance !== null &&
    hasValidHospitalLocation &&
    hasValidAmbulanceLocation;

  console.log("Should show map?", shouldShowMap);

  return (
    <div style={styles.dashboard}>
      <button onClick={resetToHome} style={styles.backButton}>
        ← Back to Home
      </button>
      ...
      {emergencyId && needsAmbulance !== null && !effectiveHospital && (
        <div>
          <p style={{ color: "yellow", textAlign: "center" }}>
      ...
      {shouldShowMap ? (
        <div>
          ...
          <AmbulanceMap
            ...
            hospitalLocation={{
              lat:
                effectiveHospital.latitude ??
                effectiveHospital.lat,
              lng:
                effectiveHospital.longitude ??
                effectiveHospital.lng,
              name: effectiveHospital.name,
            }}
            ambulanceLocation={{
        ...
        effectiveHospital && (
          ...
          >
            ...
            <ul style={{ listStyle: "none", padding: 0 }}>
              ...
              <li>
                {needsAmbulance !== null ? "✅" : "❌"} Ambulance Selection
              </li>
              <li>{effectiveHospital ? "✅" : "❌"} Hospital Selected</li>
              <li>{ambulanceData ? "✅" : "❌"} Ambulance Data</li>
              <li>
                {hasValidEmergencyLocation ? "✅" : "❌"} Emergency Location
              </li>
            </ul>
          </div>
        )
